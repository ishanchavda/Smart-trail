...
if (isset($_POST['login'])) {
	...
	$sql = "SELECT * FROM smart_rto_admin WHERE (ra_email = '" . $email. "' OR ra_username = '".$email."' )and ra_password = '" .$password. "'";

	$result = mysqli_query($conn,$sql);
	$count = mysqli_num_rows($result);
	if($count==1){
	if($row = mysqli_fetch_array($result)) {
		$_SESSION['rto_id'] = $row['ra_id'];
		$_SESSION['rto_name'] = $row['ra_username'];
		$_SESSION['rto_email'] = $row['ra_email'];
...
<head>
  ...
  <title>Log in | RTO Admin | Smart-Trail</title>
